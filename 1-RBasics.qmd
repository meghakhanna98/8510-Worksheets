...
my_number * 2 
[1] 20
the operation would multiply each element of the vector by 2, resulting in a new vector with doubled values: 4, 8, 12, and 16.
```
...
```{r}
agicalItemsList <- list("Crystal Ball", "Wizard's Wand", "Enchanted Cloak", "Potion of Wisdom", "Book of Spells")
```
...
>
```{r}
The floor() function in R is used to round down numerical values to the nearest integer.
floor(3.8)
floor(0.0001)
```
