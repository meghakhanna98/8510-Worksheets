...
library(stringr)
data("gayguides")
rel.ins.gayguides <- gayguides %>%
  filter(type == "Religious Institution") %>%  
  group_by(state) %>% 
  summarise(religious_institutions = n())  
ggplot(rel.ins.gayguides, aes(x = state, y = religious_institutions)) +  
  geom_col()
...
library(ggplot2)
library(dplyr)
data("BostonWomenVoters")  #loading all the libraries
...
data("gayguides")
rel.ins.over.time <- gayguides %>%
  filter(type == "Religious Institution") %>%  
  group_by(Year) %>% 
  summarise(religious_institutions = n())  
ggplot(rel.ins.over.time, aes(x = Year, y = religious_institutions)) +  
  geom_line() +
  geom_point()
  labs(x = "Year", y = "Number of Religious Institutions", title = "Number of Religious Institutions Over Time") +
  theme_minimal()
...
library(dplyr)
library(stringr)
data("gayguides") #loading all the libraries
...
library(stringr)
data("gayguides") 
selected_types <- c("Bars/Clubs", "Restaurant", "Hotel", "Cruising Areas", "Baths") 
...
library(DigitalMethodsData)
library(ggplot2)
data <- data.frame(recreational.data)
library(ggplot2)

data <- recreational.data

# Create a ggplot bar chart
plot <- ggplot(data, aes(x = city, y = type_of_worker)) +  
  geom_col(color = "lightgreen", fill = "yellow") +  
  labs(x = "City", y = "Population", title = "Population Trend by City") +  
  theme_minimal()  
...
(@) Plot 2 explanation: 
> Through this bar chart I intended a design to compare the workers' type sizes of various cities. Each bar represents a different city, and the height of the bar indicates the type of workers size of that city. Though, it looks a bit chaotic, but it helped me identify the population trend.
...
library(ggplot2)
data(rec)
ggplot(rec, aes(x = state, y = total_expenditures)) +
  geom_boxplot() +
