...
ac_top_terms <- ac.topics %>%
  filter(topic == 1) %>%           
  arrange(desc(beta)) %>%         
  slice_head(n = 10)               
ggplot(ac_top_terms, aes(x = beta, y = reorder(term, beta))) +
  geom_col() +
  labs(y = "Terms", x = "Beta", title = "Top 10 Words for Topic 1") +
  theme_minimal()
...
mb_texts <- readtext("mbtxt/*.txt")

mb_metadata <- read.csv("https://raw.githubusercontent.com/regan008/8510-TextAnalysisData/main/mb-metadata.csv")


mb.combined <- full_join(mb_texts, mb_metadata, by = c("doc_id" = "Filename"))


custom_stopwords <- tibble(word = c("editorial", "committee", "digitized", "to", "by", "google")) 
...
set.seed(123) 
lda_mb <- LDA(dtm_mb, k = 25, control = list(seed = 1234))

library(broom)
topics_mb <- tidy(lda_mb, matrix = "beta")

head(topics_mb)

mb.top.terms <- topics_mb %>%
  arrange(desc(beta)) %>% 
  group_by(topic) %>% slice(1:5)
...
bb_combined <- full_join(bb, bb.metadata, by = c("doc_id" = "Filename"))
...
dtm_promotion <- bb_tidy %>%
  filter(R_P_E == "P") %>%
  count(doc_id, word) %>%
  cast_dtm(document = doc_id, term = word, value = n)

dtm_reception <- bb_tidy %>%
  filter(R_P_E == "R") %>%
  count(doc_id, word) %>%
  cast_dtm(document = doc_id, term = word, value = n)

lda_promotion <- LDA(dtm_promotion, k = 10, control = list(seed = 1234))
lda_all <- LDA(dtm_all, k = 10, control = list(seed = 1234))
lda_reception <- LDA(dtm_reception, k = 10, control = list(seed = 1234))

library(broom)
topics.all <- tidy(lda_all, matrix = "beta")
head(topics.all)

topics.r <- tidy(lda_reception, matrix = "beta")
head(topics.r)

topics.p <- tidy(lda_promotion, matrix = "beta")
head(topics.p)
library(wordcloud)
library(RColorBrewer)

# All data wordcloud for Topic 2
topic_all <- topics.all %>% filter(topic == 2)
wordcloud(words = topic_all$term, freq = topic_all$beta, max.words = 100, random.order = FALSE,
    rot.per = 0.3, colors = brewer.pal(6, "Dark2"))

# Promotion wordcloud for Topic 2
topic_promotion <- topics.p %>% filter(topic == 2)
wordcloud(words = topic_promotion$term, freq = topic_promotion$beta, max.words = 100, random.order = FALSE,
    rot.per = 0.3, colors = brewer.pal(6, "Dark2"))

# Reception wordcloud for Topic 2
topic_reception <- topics.r %>% filter(topic == 2)
wordcloud(words = topic_reception$term, freq = topic_reception$beta, max.words = 100, random.order = FALSE,
    rot.per = 0.3, colors = brewer.pal(6, "Dark2"))

plot_top_terms <- function(topics_df, title = "Top Terms Across Topics") {
  top_terms <- topics_df %>%
    group_by(topic) %>%
    top_n(10, beta) %>%
    ungroup() %>%
  ...
  ggplot(top_terms, aes(x = reorder(term, beta), y = beta, fill = as.factor(topic))) +
    ...
    labs(x = "Term", y = "Beta", title = title) +
    theme_minimal() +
    theme(axis.text.x = element_text(angle = 45, hjust = 1))
}

# Plot top terms for all data
plot_top_terms(topics.all, "Top Terms Across Topics in All Data")

# Plot top terms for promotion data
plot_top_terms(topics.p, "Top Terms Across Topics in Promotion Data")

# Plot top terms for reception data
plot_top_terms(topics.r, "Top Terms Across Topics in Reception Data")
...
set.seed(123)
bb_lda <- LDA(dtm_bb, k = 10, control = list(seed = 1234))

library(broom)
topics.bb <- tidy(bb_lda, matrix = "beta")
head(topics.bb)
...
topic1 <- top_terms %>% filter(topic == 2)

wordcloud(words = topic1$term, freq = topic1$beta, max.words = 100, random.order = FALSE,
          rot.per = 0.3, colors = brewer.pal(6, "Dark2"))
...
The presence of terms like "physical", "exercise', "arms", "arm," and "raise" might suggest that this topic may relate to physical activity, possibly training or descriptions of physical feats. This could be indicative of narratives or descriptions of the athleticism of performers or characters within the Buffalo Bill's Wild West show. Terms like "forw" (possibly shorthand for "forward") and "front" suggest a focus on movement, which could relate to staging directions in promotional materials or actions in the show as described in reviews. The word "school" might point to an educational element, perhaps discussing the training of performers or an aspect of the show that had an instructive nature.

Specifically fot text analysis of promotional language, I assume the focus revolve around the central figure of Buffalo Bill and the Wild West, possibly encompassing promotional materials that highlight the main attractions of the show and Cody's persona. The term "exhibition" might indicate that this topic may cover texts related to the shows or exhibitions that are part of the Buffalo Bill Wild West spectacles.The word "west" and "fort" can point to the setting of these exhibitions, likely the American West, which is in line with the thematic content of Buffalo Bill's shows.

The terms listed in reception language list do not immediately suggest a theme to me. They are quite diverse, with words like "Michigan" "griddle" (related to cooking), "teepees" (shelter or accommodation, possibly related to Native American culture), and "burros" (animals, donkeys). Other words that came across were "Promised" and "hailed" which could relate to reception language, while "courteous" could relate to descriptions of character conduct.
