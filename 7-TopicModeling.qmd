...
ggplot(ac.top.terms, aes(x = beta, y = reorder(term, beta))) +
  geom_col() +
...
download.file("https://github.com/regan008/8510-TextAnalysisData/blob/main/MindAndBody.zip?raw=true", "MB.zip", mode = "wb")
unzip("MB.zip")
...
mb_texts <- readtext(file_paths)
mb_whole <- full_join(mb_metadata, mb_texts, by = c("Filename" = "doc_id")) %>% as_tibble()
...
dtm <- DocumentTermMatrix(Corpus(VectorSource(tidy_mb$text)), control = list(stopwords = TRUE))
...
metadata_1920s <- mb.metadata %>%
  filter(Year >= 1920 , Year <= 1929)

custom_stopwords <- c("specific", "words", "to", "remove", "basedon", "yourcorpus")

preprocessed_docs <- list()

for (i in 1:nrow(metadata_1920s)) {
  doc_path <- file.path("https://raw.githubusercontent.com/regan008/8510-TextAnalysisData/main/mb-metadata.csv", metadata_1920s$Filename[i])

  doc_text <- readtext(doc_path)$text
 
  doc_corpus <- Corpus(VectorSource(doc_text))
  
}

final_corpus_1920s <- do.call(c, preprocessed_docs)

dtm_1920s <- DocumentTermMatrix(final_corpus_1920s)

lda_model_1920s <- LDA(dtm_1920s, k = 25, control = list(seed = 1234))
lda_model_1920s
...
set.seed(123)
bb_lda <- LDA(bb_dtm, k = 10, control = list(seed = 1234))

topics(bb_lda)


bb.metadata <- bb.metadata %>%
  separate(FullDate, into = c("year", "month", "day"), sep = "-")

bb_topics_time <- bb_topics %>%
  left_join(bb.metadata, by = "doc_id") %>%
  group_by(topic, year, category) %>%
  summarize(beta_sum = sum(beta))

ggplot(bb_topics_time, aes(x = year, y = beta_sum, color = category)) +
  geom_line() +
  facet_wrap(~ topic)
