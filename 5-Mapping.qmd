...
BostonWomenVoters <- BostonWomenVoters %>%
  mutate(full.address = paste(Town.of.Birth, State.or.Province.of.Birth, Country.of.Birth, sep = ", "))

BostonWomenVoters.coordinates <- map_df(BostonWomenVoters$full.address, ~ {
  geocode_result <- geocode_OSM(.x)
 
  tibble(full.address = .x, lat = geocode_result$coords[2], lon = geocode_result$coords[1])
})


BostonWomenVoters_subset <- BostonWomenVoters.coordinates %>%
  slice(1:100)

# Display the first few rows of the subsetted data
head(BostonWomenVoters_subset)
...
address.geocoded <- streetaddresses %>% geocode(address = streetaddress, method='dsk', lat = latitude, long = longitude)
head(address.geocoded)
```
