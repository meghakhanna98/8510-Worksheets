...
BostonWomenVoters <- BostonWomenVoters%>%
  mutate(full.address = paste(Street.Number.on.April.1, Street.of.Residence.on.April.1, "Boston", "MA", sep=" "))

BostonWomenVoters.coordinates <- BostonWomenVoters%>%
map_df(BostonWomenVoters$full.address, function(address) {
  geocode_result <- geocode(address, source = "osm")
  tibble(full.address = address, lat = geocode_result$lat, lon = geocode_result$lon)
})
...
> Since the dataframe identifies latitude, longitude, group, order, region, subregion,  these columns, I think, collectively describe the shapes of geographic entities such as states or counties by providing a sequence of latitude and longitude coordinates. 
