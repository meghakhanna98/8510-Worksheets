...
> Firstly, we  took the whole State of the Union (SOTU) metadata and created into a dataframe called 'meta'. The SOTU text files stored in the directory 'sotu_files', saving them in file_paths which is necessary to load the text files for analysis. After reading, we combined the metadata with the text of the speeches. It sorts the metadata by president using arrange(), then binds the columns of the sorted metadata with the speech texts. We converted the result into a tibble (sotu_whole). The 'unnest_tokens(word, text)' function tokenizes the speeches into words, breaking down the text into individual words. This is a necessary step for most text analysis tasks, including topic modeling. Finally, we used anti_join(stop_words) which removes common stop words from the tokenized text.
...
> I think because the average attention span of people have decreased over time, influenced by the fast pace of information dissemination and consumption on digital platforms which might have led to modern presidents prioritizing clear, focused messaging on key policy goals and achievements rather than exhaustive reviews of government activities.
...
pre_tv_era <- sotu_whole %>% filter(year < 1947)
post_tv_era <- sotu_whole %>% filter(year >= 1947)
library(tidytext)

tidy_pre_tv <- pre_tv_era %>%
  unnest_tokens(word, text) %>%
  anti_join(stop_words)

tidy_post_tv <- post_tv_era %>%
  unnest_tokens(word, text) %>%
  anti_join(stop_words)

pre_tv_stats <- tidy_pre_tv %>%
  summarise(total_words = n())

post_tv_stats <- tidy_post_tv %>%
  summarise(total_words = n())
...
> 'tidy_sotu %>%' starts the chain of operations on the tidy_sotu dataset. 'count(president, word)' uses the count() function to count the occurrences of each word for each president. 'group_by(president)'groups the resulting dataframe by president We have to be using it to calculate totals and frequencies specific to each president's speeches. 'mutate(n_tot = sum(n), term_freq = n/n_tot)' adds new columns to the data frame which represents the total number of words used by each president across all their SOTU addresses, calculated with sum(n). 'term_freq=, I think calculates the term frequency of each word as the number of occurrences of that word (n) divided by the total number of words (n_tot). This might help in understanding the significance of the words in speeches. 'arrange(desc(term_freq))' orders the dataframe based on the term_freq column in descending order. Now, the words with the highest term frequency (i.e., relative importance or significance) for each president are at the top of their respective groups.The 'top_n(1)' function selects the top row(s) within each group after sorting. Because the data is grouped by president and sorted by term_freq, this effectively selects the most significant word for each president.
...
```{r}
obama_sotu <- tidy_sotu %>%
  filter(president == "Barack Obama")
obama_word_counts <- obama_sotu %>%
  count(word, sort = TRUE)
obama_tf_idf <- obama_sotu %>%
  count(doc_id, word) %>%
  bind_tf_idf(word, doc_id, n)

top_words_raw_count <- head(obama_word_counts)

top_words_tf_idf <- obama_tf_idf %>%
  arrange(desc(tf_idf)) %>%
  distinct(word, .keep_all = TRUE) %>%
  head()

```
I am assuming that through the data retrieved that there is a focus on national identity like ("american", "americans") and concerns, with emphasis on employment ("jobs") and perhaps a sense of urgency or the historical moment ("time").Through tf-idf dataset, I got different set of words where "cory" and "rebekah" could likely be personal names. I googled the term "isil" which is an acronym referring to the Islamic State of Iraq and the Levant, indicating a focus on foreign policy and security issues. Words like "voices" and "restart" can indicate overarching themes of Obama's speeches, such as inclusivity, democracy ("voices"), and recovery or change ("restart").
...
sotu_filtered_word_pairs <- sotu_word_pairs %>%
  filter(n >= 125, n <= 175)

sotu_graph <- graph_from_data_frame(sotu_filtered_word_pairs)

ggraph(sotu_graph, layout = "fr") + 
  geom_edge_link(aes(edge_alpha = n, edge_width = n), color = "pink") +
...
> The research will attempt to uncover the linguistic distinctions between promotion language and reception language and observing the evolution of this language throughout the period when Buffalo Bill's Wild West show was active.
...
bb.metadata <- read.csv("https://raw.githubusercontent.com/dseefeldt/IndianaNewspapers/main/metadata.csv")
head(bb.metadata$FullDate)

bb.metadata <- bb.metadata %>%
  mutate(Date = gsub(" .*", "", FullDate), 
         Date = as.Date(FullDate, format = "%Y/%m/%d")) %>%
  separate(FullDate, into = c("Year", "Month", "Day"), sep = "-", remove = FALSE)
...
bb_combined <- left_join(bb, bb.metadata, by = c("doc_id" = "Filename"))
...
```{r}
#1 I am loading the required libraries
library(tidytext)
library(stringr)
library(dplyr)
library(ggplot2)
...
#2 I am Prepare the text by removing punctuation and space
bb_combined <- bb_combined %>%
  mutate(text = str_replace_all(text, "[[:punct:]]", " ")) %>%
  mutate(text = str_trim(text))

#3 I am tokenizing the text into words
bb_tokens <- bb_combined %>%
  unnest_tokens(word, text)

#4 I am removing stop words including additional custom words and numbers
custom_stop_words <- c("buffalo", "cody", "bill", "wild", "west")
bb_tokens <- bb_tokens %>%
  filter(!word %in% custom_stop_words) %>%
  anti_join(stop_words, by = "word") %>%
  filter(!str_detect(word, "^[0-9]+$"))  

#5 I am calculating the word frequency
word_freq <- bb_tokens %>%
  count(word, sort = TRUE)

#6 I am calculating TF-IDF
tf_idf <- bb_tokens %>%
  count(doc_id, word) %>%
  bind_tf_idf(word, doc_id, n)

#7 I am attempting to perform sentiment analysis
bb_sentiment <- bb_tokens %>%
  inner_join(get_sentiments("bing"), by = "word") %>%
  count(doc_id, sentiment, sort = TRUE)

#8 I am joining TF-IDF results with metadata to categorize the entries
tf_idf_categorized <- tf_idf %>%
  left_join(bb.metadata, by = c("doc_id" = "Filename"))

#9 I am now separate the data into categories for Promotion, Reception, and Extraneous
promotional_df <- tf_idf_categorized %>%
  filter(R_P_E == "P")

reception_df <- tf_idf_categorized %>%
  filter(R_P_E == "R")

extraneous_df <- tf_idf_categorized %>%
  filter(R_P_E == "E")

#10 To unclutter I am extract the top 10 terms from Promotion and Reception categories
top_promotional_terms <- promotional_df %>%
  top_n(10, tf_idf)

top_reception_terms <- reception_df %>%
  top_n(10, tf_idf)

#11 I am prepare the data now for temporal analysis 
tf_idf_temporal <- tf_idf_categorized %>%
  mutate(FullDate = as.Date(FullDate, format = "%Y-%m-%d")) %>%
  separate(FullDate, into = c("Year", "Month", "Day"), sep = "-", remove = FALSE)

#12 I am preparing graph data frame for visualization 
graph <- graph_from_data_frame(tf_idf_temporal)

#13 Lastly, I am visualizing the data using ggraph 
g <- ggraph(graph, layout = "fr") + 
  geom_edge_link(aes(edge_alpha = n, edge_width = n), color = "pink") +
  geom_node_point(size = 1, color = "skyblue") +
  geom_node_text(aes(label = name), repel = TRUE, point.padding = unit(0.2, "lines")) +
  labs(title = "Word Significance Over Time",
       x = "Year",
       y = "TF-IDF") +
  theme_void()
...
I focused on the difference in promotion language versus reception language and change over time within the Buffalo Bill's Wild West show coverage for my research question.

Reception Language: Terms like "forfeit", "bombs", "carlisle", "curtner", and "flo" appeared with the highest frequency in Reception language. These terms, particularly "bombs" and "forfeit", could suggest topics of discussion that involve the drama or unexpected events associated with the shows, possibly even reflecting public sentiment or critical perspectives.

Promotional Language: Conversely, terms such as "unreadable", "bloomington", "ingraham", "date", and "bedford" were most prevalent in Promotional language. The presence of place names indicates a geographical targeting in promotional efforts, while "unreadable" could refer to the state of archival materials.
