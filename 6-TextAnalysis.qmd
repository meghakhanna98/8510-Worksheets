...
#6 Analyzing sentiments
bb_sentiment <- bb_tokens %>%
  inner_join(get_sentiments("bing"), by = "word") %>%
  count(doc_id, sentiment, sort = TRUE)

#7 I am joining the tf_idf results with metadata to get categories (Promotion, Reception, Extraneous)
tf_idf_categorized <- tf_idf %>%
  left_join(bb.metadata, by = c("doc_id" = "Filename"))

#8 I am separating analyses for Promotion and Reception
promotional_language <- tf_idf_categorized %>%
  filter(R_P_E == "P") %>%
  top_n(10, tf_idf)

reception_language <- tf_idf_categorized %>%
  filter(R_P_E == "R") %>%
  top_n(10, tf_idf)


#9 I am attempting to study the change over time analysis can be done by filtering by year and comparing frequencies
tf_idf_temporal <- tf_idf_categorized %>%
  mutate(FullDate = gsub(" .*", "", FullDate), 
         Date = as.Date(FullDate, format = "%Y/%m/%d")) %>%
  separate(FullDate, into = c("Year", "Month", "Day"), sep = "-", remove = FALSE)

tf_idf_stop_words <- tf_idf_temporal %>% add_row(word="Buffalo", "Bill", "buffalo", "bill")
#10 I am attemoting to visualize the results
library(ggplot2)
library(igraph)
library(ggraph)

tf_idf_temporal_filtered_word_pairs <- tf_idf_temporal %>%
  filter(n <= 10)

ggraph(tf_idf_temporal, layout = "fr") + 
  geom_edge_link(aes(edge_alpha = n, edge_width = n), color = "pink") +
...
I focused on the difference in promotion language versus reception language within the Buffalo Bill's Wild West show coverage for my research question.

Findings:

The promotional language was characterized by words such as [insert top words], which suggests an emphasis on [insert themes].
Reception language often included terms like "bill","buffalo","""""", indicating a focus on [insert observed aspects from receptions].
Over time, there was a shift in language from [insert observed trend] to [insert observed trend], reflecting changes in societal interests or the evolving narrative surrounding the show.
