...
average_locations_per_city <- gayguides %>%
  ...
  group_by(city) %>%
  summarize(total_locations = n(), .groups = 'drop') %>%
  summarize(average_locations = mean(total_locations))
...
joined_data <- inner_join(gayguides, regions, by = c('state'= 'State'))
locations_by_region_1980 <- joined_data %>%
  filter(Year == 1980) %>%
  group_by(Division) %>%
  summarise(total_locations = n())

mountain_region_1980 <- filter(locations_by_region_1980, Division == "Mountain")
